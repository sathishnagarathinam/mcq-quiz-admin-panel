...
exports.updateAnalytics = exports.generateDailyChallenge = exports.onGeneralFeedbackResponse = exports.onFeedbackResponse = exports.onQuestionCreate = exports.onQuizComplete = exports.onUserCreate = exports.api = void 0;
...
// Trigger when general feedback is updated with admin response
exports.onGeneralFeedbackResponse = functions.firestore
    .document('general_feedback/{feedbackId}')
    .onUpdate(async (change, context) => {
    try {
        const beforeData = change.before.data();
        const afterData = change.after.data();
        ...
        const statusChanged = beforeData.status !== 'responded' && afterData.status === 'responded';
        if (wasResponded && statusChanged) {
            console.log(`Admin responded to general feedback ${context.params.feedbackId}`);
            // Create notification for the user
            await createGeneralFeedbackResponseNotification(afterData, context.params.feedbackId);
        }
    }
    catch (error) {
        console.error('Error processing general feedback response:', error);
    }
});
// Scheduled Functions
...
async function createGeneralFeedbackResponseNotification(feedbackData, feedbackId) {
    var _a;
    try {
        console.log(`Creating notification for general feedback response: ${feedbackId}`);
        // Get user's FCM token
        const userDoc = await admin.firestore().collection('users').doc(feedbackData.userId).get();
        if (!userDoc.exists) {
            console.log(`User ${feedbackData.userId} not found`);
        ...
        const notificationData = {
            title: 'Admin Response to Your Feedback',
            body: `We've responded to your feedback about "${feedbackData.subject}". Check it out!`,
            imageUrl: null,
            actionUrl: `/general-feedback/${feedbackId}`,
            actionType: 'navigate',
            target: {
                type: 'specific_users',
                userIds: [feedbackData.userId]
            ...
            sentAt: admin.firestore.FieldValue.serverTimestamp(),
            priority: 'normal',
            category: 'general_feedback_response',
            metadata: {
                feedbackId: feedbackId,
        ...
        if (userData.fcmToken) {
            const message = {
                ...
                data: {
                    type: 'general_feedback_response',
                    feedbackId: feedbackId,
                    category: feedbackData.category,
                    actionUrl: notificationData.actionUrl
                },
        ...
        console.log(`General feedback response notification created successfully for user ${feedbackData.userId}`);
    }
    catch (error) {
        console.error('Error creating general feedback response notification:', error);
    }
}
